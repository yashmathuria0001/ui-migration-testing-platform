...
test('AI Generated Test', async ({ page }) => {
    const screenshotDir = require('path').join('screenshots', process.env.ENV_TYPE || 'run');
    require('fs').mkdirSync(screenshotDir, { recursive: true });
    await page.screenshot({ path: require('path').join(screenshotDir, `${Date.now()}.png`), fullPage: true });

  await page.screenshot({ path: `screenshots/${process.env.RUN_LABEL || 'run'}_${Date.now()}.png`, fullPage: true });
  ...
  await test.step('2. Enter username tomsmith into the Username field', async () => {
    await page.fill('input[name="username"]', 'tomsith');
    const dir = path.join('screenshots', process.env.RUN_LABEL || 'run', process.env.TEST_RUN_ID || 'unknown');
    fs.mkdirSync(dir, { recursive: true });
